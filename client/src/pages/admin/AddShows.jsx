...
const AddShows = () => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const showInputs = Object.entries(dateTimeSelection).map(([date, time]) => ({ date, time }));
        ...
        <>
            ...
            <div className='overflow-x-auto pb-4'>
                <div className="group flex flex-wrap gap-4 mt-4 w-max">
                    {nowPlaying.map((movie, index) => (
                        <div key={`${movie}${index}`} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`} onClick={() => setSelectedMovie(prev =>
            ...
            <div className="mt-6">
                ...
                <div className="inline-flex gap-5 border border-gray-600 p-1 pl-3">
                    <input type="datetime-local" value={dateTimeInput} onChange={(e) => setDateTimeInput(e.target.value)} className='text-white outline-none rounded-md' />
                    <button onClick={handleDateTimeAdd} className="bg-primary text-white px-3 py-2 text-sm rounded-lg hover:bg-primary cursor-pointer">
                        Add Time
                    </button>
                </div>
            </div>
